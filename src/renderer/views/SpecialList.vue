<template>
  <div class="v-row list-view" @click="onListViewClicked">
    <div
      ...
      :key="tag.id"
      :data-id="tag.id"
      :class="{ active: specialTagParam.id === tag.id }"
    >
      {{ tag.name }}
    </div>
...
import { ComputedPage, Special, SpecialTags, Tag } from '@/types';
    ...
    return {
      ...
      onListViewClicked(event: PointerEvent | MouseEvent) {
        ...
        if (value && value !== specialTagParam.id) {
          ...
          getSpecialList(page, specialTagParam)
            .then(data => {
              data.page && Object.assign(page, data.page);
              specialList.splice(0, specialList.length, ...data.list);
            })
