<template>
  <div class="v-row list-view" v-for="(children, index) in tags" :key="index" @click="onListViewClicked">
    <div
      ...
      :key="tag.id"
      :data-tag="tag.value"
      :class="{ active: tag.id === singerTagParam[tag.group as 'en'] }"
    >
      {{ tag.name }}
    </div>
  </div>
...
import { ComputedPage, Singer, SingerTags, SingerTagsParam } from '@/types';
  ...
  setup() {
    ...
    return {
      ...
      onListViewClicked(event: PointerEvent | MouseEvent) {
        ...
        Spinner.open();
        getSingerList(page, singerTagParam)
          .then(data => {
            // 重设分页信息
            data.page && Object.assign(page, data.page);
            // 添加歌手数据
