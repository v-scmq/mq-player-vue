...
export default {
  install(app) {
      ...
      new Promise(resolve => {
        ...
        if (options.data) {
          options.headers["Content-Type"] =
            typeof options.data === "object" || options.headers.postJSON
        ...
        window.electron
          ...
          .then(res => {
            app.config.globalProperties.$message(
              `状态码：${res.statusCode}, 消息：${res.statusMessage}`
            );
          ...
          .finally(() =>
            postJSON ? (options.headers.postJSON = postJSON) : null
          );
      });
    ...
    app.config.globalProperties.$source = {
      /* KuGouSource,*/ QQMusicSource,
      impl: QQMusicSource
    };
