 ...
 */
export const useMediaPlayer = (): [typeof player, typeof playIndex, Song[]] => [player, playIndex, playList];
...
export const playMediaList = (list: Song[], index: number) => {
  for (const song of list) {
    song.singerName = song.singer instanceof Array ? song.singer.join('/') : (song.singer as string);
  }

  playList.splice(0, playList.length, ...list);
  playIndex.value = index;

  if (!list || list.length < 1) throw new Error('指定的媒体资源列表必须包含媒体信息!');
  if (index < 0 || index > list.length) throw new Error('指定的播放索引无效!');
