...
export const db = {
    ...
    delete(table: DataTable, id: number | string) {
        return new Promise(resolve => {
            ...
            const request = database.transaction([table.name], 'readwrite')
                .objectStore(table.name).delete(id);
    ...
    update<T extends RowData>(table: DataTable, data: T): Promise<boolean> {
        return new Promise(resolve => {
            ...
            const request = database.transaction([table.name], 'readwrite')
                .objectStore(table.name).put(data);
    ...
    query<T extends RowData>(table: DataTable, id: string | number): Promise<T[]> {
        return new Promise(resolve => {
            ...
            const request: IDBRequest<T[]> =
                database.transaction([table.name], 'readonly')
